...
import states from "@/lib/utils/states.json";
...
const LocationSelector = ({
  ...
  useEffect(() => {
    if (value?.country) {
      const country = countriesData.find(
        (c) =>
          c.name.toLowerCase() === value.country?.toLowerCase()
      );
      if (country) {
        setSelectedCountry(country);
      }
    }
  }, [value?.country, countriesData]);

  // Handle state selection after country is set
  useEffect(() => {
    if (value?.state && selectedCountry) {
  ...
  }, [value?.state, selectedCountry, statesData]);
